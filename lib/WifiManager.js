...
module.exports = class WifiManager{
	...
	EnableAccessPoint(){
		...
		this._copyFile('../templates/hostapd/hostapd.ap.template', '/etc/dnsmasq.conf');
		this._copyFile('../templates/default/hostapd.ap.template', '/etc/default/hostapd');

		this._run('iptables -t nat -A PREROUTING -p tcp --dport 80 -j DNAT --to-destination localhost:80');
	...
	EnableWifi(){
		...
		this._copyFile('../templates/hostapd/hostapd.station.template', '/etc/dnsmasq.conf');
		this._copyFile('../templates/default/hostapd.station.template', '/etc/default/hostapd');

		this._run('iptables -t nat -D PREROUTING -p tcp --dport 80 -j DNAT --to-destination localhost:80');
