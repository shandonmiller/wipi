...
module.exports = class WifiManager{
	...
	EnableWifi(ssid, password){
		this._writeWPASupplicant(ssid, password);
	...
	_writeWPASupplicant(ssid, password){
		let wpa_supplicant = fs.readFileSync('./templates/wpa_supplicant/wpa_supplicant.conf.template', 'UTF-8');
     	let compiled = compile(wpa_supplicant);
     	let content = resolveToString(compiled, {ssid, password});
     	fs.writeFileSync('/etc/wpa_supplicant/wpa_supplicant.conf', content);
	}
